// Mock product data for fallback demonstrations
// Mock product data for fallback demonstrations
const MOCK_PRODUCTS = [
  {
    _id: "prod001",
...
// Load products from API, fallback to Mock data if API is offline
async function loadProducts() {
  const productsGrid = document.querySelector('.products-grid');
  productsGrid.innerHTML = '<div class="text-center" style="grid-column: 1/-1;">Loading products...</div>';

  try {
    const fetched = await API.get('/products?limit=100', false);
    ...
    if (allProducts.length === 0) {
      console.warn('API returned 0 products, showing mock fallback database...');
      allProducts = MOCK_PRODUCTS;
    }
  } catch (error) {
    console.warn('API error, falling back to mock product database:', error.message);
    allProducts = MOCK_PRODUCTS;
  }

  // Sync initial state from URL query parameter
  const searchKeyword = getQueryParam('search') || '';
  if (searchKeyword) {
    const knownCategories = ["Electronics", "Footwear", "Accessories", "Bags", "Men's Wear", "Women's Wear"];
    const matchedCategory = knownCategories.find(c => c.toLowerCase() === searchKeyword.toLowerCase());
    ...
    if (matchedCategory) {
      ...
      // Check the matched category checkbox
      const catCheckboxes = document.querySelectorAll('.category-checkbox');
      catCheckboxes.forEach(cb => {
        if (cb.value.toLowerCase() === searchKeyword.toLowerCase()) {
          cb.checked = true;
        }
      });
    } else {
      // Set value in the search bar
...
function renderProductGrid(products) {
  ...
  if (products.length === 0) {
    productsGrid.innerHTML = '<div class="text-center" style="grid-column: 1/-1;">No products found matching the criteria.</div>';
    return;
  }
  ...
  products.forEach(product => {
    ...
    card.innerHTML = `
      <a href="product-details.html?id=${product._id}" class="product-img-wrapper">
        <img src="${product.image}" alt="${product.name}" class="product-img" loading="lazy" onerror="this.src=getCategoryFallbackImage('${product.category}')">
      </a>
      <div class="product-info">
        ...
        <div class="product-footer">
          ...
          <button class="btn-add-cart-icon" data-id="${product._id}" data-name="${encodeURIComponent(product.name)}" data-price="${product.price}" data-image="${product.image}" data-category="${encodeURIComponent(product.category)}" title="Add to Cart">
            🛒
          </button>
        </div>
      </div>
...
// Apply checked category/price filters and render matching products
function applyFilters() {
  ...
  if (catAll && !catAll.checked) {
    ...
    activeCategories = Array.from(checkboxes).map(cb => cb.value);
  }

  // Get active price range
  const priceRadio = document.querySelector('input[name="price"]:checked');
  const priceVal = priceRadio ? priceRadio.value : 'any';

  let filtered = allProducts;

  // 1. Search Filter (checks title or category)
  if (searchVal) {
  ...
  // 3. Price Filter (converted to local Rs. magnitude equivalents: 50 USD -> Rs. 4,000, 150 USD -> Rs. 12,000)
  if (priceVal === 'under-50') {
    filtered = filtered.filter(p => p.price < 50);
  } else if (priceVal === '50-150') {
...
function handleQuickAdd(e) {
  ...
  const product = {
    ...
    category: decodeURIComponent(btn.dataset.category || 'Footwear'),
    quantity: 1
  };
...
// Core addToCart logic (localStorage first)
function addToCart(itemToAdd) {
  let cartItems = JSON.parse(localStorage.getItem('cartItems')) || [];
  const existItem = cartItems.find(x => x.product === itemToAdd.product);
...
// Set up UI filter action triggers
function setupCatalogFilters() {
  ...
  if (catAll) {
    catAll.addEventListener('change', () => {
      // Clear search input & clean URL query parameters to prevent conflicts
      ...
      if (catAll.checked) {
        ...
        if (noneChecked) catAll.checked = true;
      }
      applyFilters();
    });
  }
  ...
  catCheckboxes.forEach(cb => {
    cb.addEventListener('change', () => {
      ...
      if (cb.checked) {
        // Uncheck all other categories to act like radio choices
        catCheckboxes.forEach(otherCb => {
          if (otherCb !== cb) otherCb.checked = false;
        });
        if (catAll) catAll.checked = false;
      } else {
        const noneChecked = Array.from(catCheckboxes).every(c => !c.checked);
        if (noneChecked && catAll) catAll.checked = true;
      }
      applyFilters();
    });
  });
  ...
  if (searchForm && searchInput) {
    ...
    searchInput.addEventListener('input', applyFilters); // Real-time search as user types!
  }
}
